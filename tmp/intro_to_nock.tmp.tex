...
\kode{*x}?  Looking up, we see that lines 23 through 39 reduce \kode{*x}---
for different patterns of \kode{x}.
...
So \kode{*42} is \kode{*42}, which is\ldots{} \kode{*42}.  Logically, Nock goes into
...
to every node in the tree.  The root of the tree is \kode{/1}.  The
left child of every node at \kode{/a} is \kode{/2a}; the right child is
\kode{/2a+1}.  (Writing \kode{(a + a)} is just a clever way to write \kode{2a},
